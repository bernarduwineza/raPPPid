...
if bool_BATCH_PROC          % batch processing
    ...
    % get settings of main agent
    ROW_MAIN_AGENT = TABLE(2,:);
    settings_main_agent = BatchProcessingPreparation(settings, ROW_MAIN_AGENT);

    if checkProcessingSettings(settings_main_agent, false)
        [mainAgentData] = InitializeAgentProcessing(settings_main_agent);
    else
        STOP_CALC = 0;
    end
    ...
    [~,file,ext] = fileparts(settings_main_agent.INPUT.file_obs);
    fprintf('\n---------------------------------------------------------------------\n');
    ...
    for q = mainAgentData.q_range
        ...
        [mainAgentData] = ProcessAgentEpoch(mainAgentData, q);
