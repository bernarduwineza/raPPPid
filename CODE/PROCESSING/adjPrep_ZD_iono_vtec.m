function [Epoch, Adjust] = adjPrep_ZD_iono_vtec(settings, Adjust, Epoch, prns_old, elevs_old, obs_int)

% Preparations for Zero-Difference-adjustment with (constrained) estimation 
% of ionospheric VTEC. Function is executed before position is calculated with 
...
% indices of the vtec
K = settings.IONO.Bspline.K; % number of Bspline coefficients
total_params = NO_PARAM+no_ambiguities+K;  
iono_idx = (1+NO_PARAM+no_ambiguities):(total_params); 
%---DEBUG_CLK
total_params = NO_PARAM+no_ambiguities+K + 1; % the additional parameter is the clock drift
iono_idx = (1+NO_PARAM+no_ambiguities):(total_params-1); 
%---
...
%   --- first epoch of processing or no float solution
    ...
    param_vec = zeros(total_params, 1);  % 16 + no. ambiguities + no. vtec grid parameters + clock drift
    ...
    param_sigma = eye(total_params);	% initialize
    ...
    param_sigma(iono_idx,iono_idx) = ...                    % ionospheric delay
        eye(K)*FILTER.var_iono;
        
    %---DEBUG_CLK
    param_sigma(total_params,total_params) = 1e3;   % receiver clock drift
    %---
...
%   --- delete ambiguities of vanished satellites
        ...
        if ~isempty(del_idx)                % remove ambiguities and ionospheric delay of vanished satellites
            ...
            if bool_code_phase      % code and phase processing: 
                % ... ambiguities have to be removed
...
%   --- for all epochs with valid float solution
    ...
    Noise(iono_idx,iono_idx) = iono_eye * FILTER.Q_iono_vtec * dt;   % add noise of ionospheric delays
    if ZWD_ON
        Noise(4,4) = FILTER.var_zwd * dt;    	% if ZWD estimation is not started in first epoch
    end
    %---DEBUG_CLK
    Noise(total_params, 1:total_params) = zeros(1,total_params);
    Noise(1:total_params,total_params)  = zeros(1,total_params)';
    Noise(total_params, total_params)   = (1)^2 * dt;
    %---
    ...
    Transition(iono_idx,iono_idx) = iono_eye*FILTER.dynmodel_iono_vtec; % add transition of ionospheric delay

    %---DEBUG_CLK
    Transition(total_params, 1:total_params) = zeros(1,total_params);
    Transition(1:total_params, total_params) = zeros(1,total_params)'; 
    Transition(total_params,total_params) = 1;
    Transition(5,total_params) = obs_int;                               % add transition of clock drift
    %---
