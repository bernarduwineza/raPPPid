function [lat_lon_bases] = compute_bspline_bases(settings, lat_pp, lon_pp)
...
for i = 1:kj1
    lat_bases(i, :) = bspline_basis(i, lat_degree, lat_knots, lat_pp*180/pi);
end
for i = 1:kj2
    lon_bases(i, :) = bspline_basis(i, lon_degree, lon_knots, lon_pp*180/pi);
end
lat_lon_bases = zeros(length(lat_bases(:,1))*length(lon_bases(:,1)), length(lat_pp));
for i = 1:length(lat_pp)
    lat_lon_bases(:,i) = kron(lon_bases(:,i), lat_bases(:,i));
end
end